...
public interface DeliveryManagerRepository extends JpaRepository<DeliveryManager, UUID> {
    ...
    @Query(value = "SELECT * FROM p_delivery_manager WHERE manager_type = 'HUB' AND deleted_at IS NULL ORDER BY sequence LIMIT 1", nativeQuery = true)
    Optional<DeliveryManager> findFirstHubManager();

    @Query(value = "SELECT * FROM p_delivery_manager WHERE hub_id = CAST(:hubId AS UUID) AND manager_type = 'COMPANY' AND deleted_at IS NULL AND sequence > :lastSequence ORDER BY sequence LIMIT 1", nativeQuery = true)
    Optional<DeliveryManager> findNextCompanyManager(@Param("hubId") UUID hubId, @Param("lastSequence") Integer lastSequence);

    @Query(value = "SELECT * FROM p_delivery_manager WHERE hub_id = CAST(:hubId AS UUID) AND manager_type = 'COMPANY' AND deleted_at IS NULL ORDER BY sequence LIMIT 1", nativeQuery = true)
